...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Detail Transkrip Nilai</h1>
                <p class="text-gray-600 text-sm">Informasi lengkap transkrip mahasiswa</p>
            </div>
        </div>
    </div>
        ...
        <div class="flex items-center space-x-6">
            ...
            <div class="flex-shrink-0">
                <div class="text-center">
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ $user->transkripNilai->sum('sks') }}</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                {{ count($user->transkripNilai) }} Mata Kuliah
            </span>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Nilai Huruf</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Nilai Angka</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($user->transkripNilai as $i => $transkrip)
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $i + 1 }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $transkrip->mata_kuliah }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                @if($transkrip->nilai_huruf == 'A') bg-green-100 text-green-800
                                @elseif($transkrip->nilai_huruf == 'B') bg-blue-100 text-blue-800
                                @elseif($transkrip->nilai_huruf == 'C') bg-yellow-100 text-yellow-800
                                @elseif($transkrip->nilai_huruf == 'D') bg-orange-100 text-orange-800
                                @else bg-red-100 text-red-800 @endif">
                                {{ $transkrip->nilai_huruf }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="text-sm font-semibold text-gray-900">{{ $transkrip->nilai_angka }}</span>
                        </td>
                    </tr>
                    @empty
                    <tr>
