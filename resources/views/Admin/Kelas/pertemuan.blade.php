...
<div class="container-mobile p-3 sm:p-4 md:p-6 lg:p-8">
    ...
    <div class="header-mobile flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-4">
        <div>
            <div class="flex items-center gap-2 mb-2">
                <a href="/admin/kelas" class="text-blue-600 hover:text-blue-800 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                    </svg>
                </a>
                <h1 class="header-title text-xl sm:text-2xl font-bold text-gray-800">Pertemuan Kelas {{ $kelas->kelas }}</h1>
            </div>
            <p class="text-sm text-gray-600">Kelola pertemuan untuk kelas {{ $kelas->kelas }}</p>
        </div>
    ...
    <div class="hidden md:block bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div class="table-responsive">
            <table id="pertemuan-table" class="w-full table-auto">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-left py-3 sm:py-4 px-3 sm:px-6 text-xs sm:text-sm font-semibold text-gray-600 uppercase tracking-wider">Nama Pertemuan</th>
                        ...
                        <th class="text-left py-3 sm:py-4 px-3 sm:px-6 text-xs sm:text-sm font-semibold text-gray-600 uppercase tracking-wider">Tanggal</th>
    ...
    <!-- Cards Mobile/Tablet -->
    <div id="mobile-pertemuan-cards" class="md:hidden space-y-4">
...
<!-- Modal Add/Edit Pertemuan -->
<div id="pertemuan-modal" class="fixed inset-0 modal-backdrop hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-1/2 lg:w-1/3 shadow-2xl rounded-lg bg-white">
        <div class="modal-content">
            <!-- Modal Header -->
            <div class="flex justify-between items-center pb-4 mb-4 border-b border-gray-200">
                <h3 id="modal-title" class="text-lg font-semibold text-gray-900">Tambah Pertemuan</h3>
            ...
            <!-- Form -->
            <form id="pertemuan-form">
                <input type="hidden" id="pertemuan-id">

                <div class="mb-4">
                    <label for="mata-kuliah-select" class="block text-sm font-medium text-gray-700 mb-2">Mata Kuliah</label>
                    <select id="mata-kuliah-select" name="mata_kuliah_id" required
                ...
                <div class="mb-4">
                    <label for="nama-pertemuan" class="block text-sm font-medium text-gray-700 mb-2">Nama Pertemuan</label>
                    <input type="text" id="nama-pertemuan" name="nama_pertemuan" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Masukkan nama pertemuan">
                    <div id="nama-pertemuan-error" class="text-red-500 text-sm mt-1 hidden"></div>
                </div>

                <div class="mb-4">
                    <label for="tanggal-pertemuan" class="block text-sm font-medium text-gray-700 mb-2">Tanggal</label>
                    <input type="date" id="tanggal-pertemuan" name="tanggal" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <div id="tanggal-error" class="text-red-500 text-sm mt-1 hidden"></div>
                </div>
...
<script>
    ...
    let isEdit = false;
    const kelasId = parseInt('{{ $kelas->id }}');

    // Load data
    function loadPertemuanTable() {
        fetch(`/admin/kelas/${kelasId}/pertemuan/data`, {
                headers: {
            ...
            .then(data => {
                if (Array.isArray(data)) {
                    ...
                    const tbody = document.querySelector('#pertemuan-table tbody');
    ...
    function loadMataKuliah() {
        fetch(`/admin/kelas/${kelasId}/mata-kuliah`, {
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                }
            })
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    select.innerHTML += `<option value="${item.id}">${item.mata_kuliah} - ${item.semester} (${item.dosen_name})</option>`;
                });
            })
            .catch((error) => {
    ...
    function renderTable() {
        const tbody = document.querySelector('#pertemuan-table tbody');
        const mobileContainer = document.querySelector('#mobile-pertemuan-cards');
        ...
        dataToShow.forEach((item, index) => {
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <td class="py-3 sm:py-4 px-3 sm:px-6">
                        <div class="text-gray-900 text-sm sm:text-base font-medium">${item.nama_pertemuan}</div>
                    </td>
                    <td class="py-3 sm:py-4 px-3 sm:px-6">
                        <div class="text-gray-700 text-sm">${item.mata_kuliah}</div>
                        <div class="text-gray-500 text-xs">${item.semester}</div>
                    </td>
                    <td class="py-3 sm:py-4 px-3 sm:px-6">
                        <div class="text-gray-700 text-sm">${item.dosen_name}</div>
                    </td>
                    <td class="py-3 sm:py-4 px-3 sm:px-6">
                        <div class="text-gray-700 text-sm">${item.tanggal}</div>
                    </td>
                    <td class="py-3 sm:py-4 px-3 sm:px-6 text-center">
                        <div class="flex items-center justify-center space-x-1">
                            <button class="inline-flex items-center justify-center w-8 h-8 rounded-full border border-blue-300 text-blue-500 hover:bg-blue-50 transition-colors duration-200 absensi-btn" 
                                    data-id="${item.id}" 
                                    data-name="${item.nama_pertemuan}"
                                    title="Lihat Absensi">
                ...
                <div class="mobile-card p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                    <div class="flex items-start justify-between">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="text-gray-900 text-base font-medium mb-1">${item.nama_pertemuan}</div>
                            <div class="text-gray-600 text-sm mb-1"><strong>Mata Kuliah:</strong> ${item.mata_kuliah}</div>
                            <div class="text-gray-600 text-sm mb-1"><strong>Dosen:</strong> ${item.dosen_name}</div>
                            <div class="text-gray-600 text-sm"><strong>Tanggal:</strong> ${item.tanggal}</div>
                        </div>
                        <div class="flex-shrink-0 flex space-x-1">
                            <button class="inline-flex items-center justify-center w-9 h-9 rounded-full border border-blue-300 text-blue-500 hover:bg-blue-50 transition-colors duration-200 absensi-btn" 
                                    data-id="${item.id}" 
                                    data-name="${item.nama_pertemuan}"
                                    title="Lihat Absensi">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    function filterData(searchTerm) {
        filteredData = allData.filter(item =>
            item.nama_pertemuan.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.mata_kuliah.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item.dosen_name.toLowerCase().includes(searchTerm.toLowerCase())
        );
        currentPage = 1;
        renderTable();
        updatePagination();
    ...
    // Modal functions
    function showModal(isEditMode = false, pertemuanData = null) {
        isEdit = isEditMode;
        const modal = document.getElementById('pertemuan-modal');
        const title = document.getElementById('modal-title');
        const submitBtn = document.getElementById('submit-btn');

        if (isEdit && pertemuanData) {
            title.textContent = 'Edit Pertemuan';
            submitBtn.textContent = 'Perbarui';

            // Load detail untuk mengisi form
            fetch(`/admin/kelas/${kelasId}/pertemuan/${pertemuanData.id}`)
                .then(res => res.json())
                .then(response => {
                    if (response.success) {
                        const data = response.data;
                        document.getElementById('pertemuan-id').value = data.id;
                        document.getElementById('mata-kuliah-select').value = data.mata_kuliah_id;
                        document.getElementById('nama-pertemuan').value = data.nama_pertemuan;
                        document.getElementById('tanggal-pertemuan').value = data.tanggal;
                    }
                });
        } else {
            title.textContent = 'Tambah Pertemuan';
        ...
        document.getElementById('mata-kuliah-select').focus();
    }
    ...
    function submitForm(event) {
        ...
        const formData = new FormData(document.getElementById('pertemuan-form'));
        const pertemuanId = document.getElementById('pertemuan-id').value;
        ...
        const url = isEdit ? `/admin/kelas/${kelasId}/pertemuan/${pertemuanId}` : `/admin/kelas/${kelasId}/pertemuan`;
        const method = isEdit ? 'PUT' : 'POST';

        const data = {
            mata_kuliah_id: formData.get('mata_kuliah_id'),
            nama_pertemuan: formData.get('nama_pertemuan'),
    ...
    function editPertemuan(pertemuanId) {
        // Find pertemuan data from allData
        const pertemuanData = allData.find(item => item.id == pertemuanId);
        if (pertemuanData) {
            showModal(true, pertemuanData);
        }
    }

    function deletePertemuan(pertemuanId, pertemuanName) {
        if (confirm(`Apakah Anda yakin ingin menghapus pertemuan "${pertemuanName}"?`)) {
            // Get CSRF token
            const token = document.querySelector('meta[name="csrf-token"]');
            if (!token) {
                alert('CSRF token not found');
            ...
            fetch(`/admin/kelas/${kelasId}/pertemuan/${pertemuanId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': token.getAttribute('content'),
                        'X-Requested-With': 'XMLHttpRequest'
                ...
                .then(response => {
                    if (response.success) {
                        ...
                        alert(response.message || 'Gagal menghapus pertemuan');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('pertemuan-form').addEventListener('submit', submitForm);

        // Modal backdrop click
        document.getElementById('pertemuan-modal').addEventListener('click', function(e) {
            if (e.target === this) {
                hideModal();
            }
        });

        // Event delegation for absensi, edit and delete buttons
        document.addEventListener('click', function(e) {
            if (e.target.closest('.absensi-btn')) {
                const pertemuanId = e.target.closest('.absensi-btn').getAttribute('data-id');
                const pertemuanName = e.target.closest('.absensi-btn').getAttribute('data-name');
                window.location.href = `/admin/pertemuan/${pertemuanId}/absensi`;
            }

            if (e.target.closest('.edit-btn')) {
                const pertemuanId = e.target.closest('.edit-btn').getAttribute('data-id');
                editPertemuan(pertemuanId);
            }

            if (e.target.closest('.delete-btn')) {
                const pertemuanId = e.target.closest('.delete-btn').getAttribute('data-id');
                const pertemuanName = e.target.closest('.delete-btn').getAttribute('data-name');
                deletePertemuan(pertemuanId, pertemuanName);
            }
        });
